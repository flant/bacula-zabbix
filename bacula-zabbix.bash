...
case $baculaDbSgdb in
  P) export PGPASSWORD=$baculaDbPass;sql="/usr/bin/psql -At -h$baculaDbAddr -p$baculaDbPort -U$baculaDbUser -d$baculaDbName -c" ;;
...
$zabbixSender -z $zabbixSrvAddr -p $zabbixSrvPort -s $zabbixHostName -k "bacula.$level.job.status" -o $status >/dev/null 2>&1
if [ $? -ne 0 ] ; then return=$(($return+1)) ; fi
echo "$zabbixSender -z $zabbixSrvAddr -p $zabbixSrvPort -s $zabbixHostName -k "bacula.$level.job.status" -o $status"
...
$zabbixSender -z $zabbixSrvAddr -p $zabbixSrvPort -s $zabbixHostName -k "bacula.$level.job.bytes" -o $baculaJobBytes >/dev/null 2>&1
...
$zabbixSender -z $zabbixSrvAddr -p $zabbixSrvPort -s $zabbixHostName -k "bacula.$level.job.files" -o $baculaJobFiles >/dev/null 2>&1
...
$zabbixSender -z $zabbixSrvAddr -p $zabbixSrvPort -s $zabbixHostName -k "bacula.$level.job.time" -o $baculaJobTime >/dev/null 2>&1
...
$zabbixSender -z $zabbixSrvAddr -p $zabbixSrvPort -s $zabbixHostName -k "bacula.$level.job.speed" -o $baculaJobSpeed >/dev/null 2>&1
...
$zabbixSender -z $zabbixSrvAddr -p $zabbixSrvPort -s $zabbixHostName -k "bacula.$level.job.compr" -o $baculaJobCompr >/dev/null 2>&1
